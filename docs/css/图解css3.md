...
合并的写法border: border-width border-sytle(需要指定值) border-color,  
...
## 盒子阴影属性 box-shadow  
`box-shadow:none | [inset x-offset y-offset blur-raduis spread-raduis color], [inset x-offset y-offset blur-raduis spread-raduis color]`  
x-offset在x轴上的偏移量，y-offset在y轴上的偏移量(y轴指向向下),inset(内阴影，可选值，不设置则为外阴影),blur-reduis模糊半径，可选值,spread-raduis扩展半径，可选值  
另外内阴影在图片元素上无效  
